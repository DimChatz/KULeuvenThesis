...
from models import lengthFinder, ECGDataset2_0, ECGCNNClassifier, Gated2TowerTransformer, MLSTMFCN, CNN2023Attia
...
def CVtrain(modelStr, learningRate, epochs, classWeights, earlyStopPatience, 
    ...
    for foldNum in range(10):
        ...
        trainFileList, valFileList, testFileList, trainNotes = lengthFinder(f"/home/tzikos/Desktop/Data/Berts final/{experiment}/", foldNum)
        if usePretrained:
        ...
        for i in range(classTestF1.size(0)):
            ...
            wandb.log({"Test F1": epochTestF1}, step=epoch+1, commit=False)
            for i in range(len(expList)):
                wandb.log({f"Test F1 {expList[i].split(" ")[0]}": classTestF1[i]}, step=epoch+1, commit=False)
        # Create a W&B Table with appropriate column names
        # only for Berts data
        if f"AVNRT {experiment}" in expList:
            classNames = expList
...
def train(modelStr, learningRate, classWeights, expList, dataset, batchSize, modelWeightPath, 
          epochs=1000, earlyStopPatience=12, reduceLRPatience=5, swin=False, trainNotes="No notes"):
    trainBeginTime = time.time()
    # Get date for saving model weights
    now = datetime.now()
    formattedNow = now.strftime("%d-%m-%y-%H-%M")
